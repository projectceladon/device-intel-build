...
publish: publish_mkdir_dest $(PUBLISH_GOALS) publish_qemu_scripts  publish_ifwi publish_gptimage publish_firmware_symbols $(PUB_OSAGNOSTIC_TAG) publish_kf4abl_symbols $(PUB_CMCC_ZIP) publish_androidia_image publish_grubinstaller
	...
	$(hide) cp -r $(PRODUCT_OUT)/caas.img $(TOP)/pub/$(TARGET_PRODUCT)/$(TARGET_BUILD_VARIANT)/Release_Files
	...
	$(hide) tar --checkpoint=1000 --checkpoint-action=dot -czf $(TARGET_PRODUCT)-releasefiles-$(TARGET_BUILD_VARIANT).tar.gz scripts patches caas-flashfiles-*.zip caas-ota-*.zip caas.img
	...
	$(hide) rm -rf $(TOP)/$(TARGET_PRODUCT)-releasefiles-$(TARGET_BUILD_VARIANT).tar.gz && rm -rf $(TOP)/Release_Files && rm -rf $(TOP)/caas-flashfiles*.zip && rm -rf $(TOP)/scripts && rm -rf $(TOP)/patches && rm -rf $(TOP)/caas-ota-*.zip && rm -rf $(TOP)/caas.img
