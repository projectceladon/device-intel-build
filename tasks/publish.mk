...
PUB_RELEASE_FILES := Release_Files

.PHONY: $(PUB_RELEASE_FILES)
$(PUB_RELEASE_FILES): $(releasefiles)
	...
	$(hide) tar --checkpoint=1000 --checkpoint-action=dot -czf Release.tar.gz Release_Files
	$(hide) cp -r $(TOP)/Release.tar.gz $(TOP)/pub/$(TARGET_PRODUCT)/$(TARGET_BUILD_VARIANT)
	$(hide) rm -rf $(TOP)/Release.tar.gz && rm -rf $(TOP)/Release_Files
	@echo "Release files are published"
.PHONY: publish_release_files
publish_release_files: $(PUB_RELEASE_FILES)
...
publish_ci: publish_liveimage publish_qemu_scripts  publish_ota_flashfile publish_gptimage publish_grubinstaller publish_ifwi publish_firmware_symbols $(PUB_OSAGNOSTIC_TAG) $(PUB_CMCC_ZIP) $(PLATFORM_RMA_TOOLS_ZIP)
	...
	$(hide) mv $(TOP)/pub/$(TARGET_PRODUCT)/$(TARGET_BUILD_VARIANT)/Release_Files/host $(TOP)/pub/$(TARGET_PRODUCT)/$(TARGET_BUILD_VARIANT)/Release_Files/patches
	$(hide) cp -r $(TOP)/pub/$(TARGET_PRODUCT)/$(TARGET_BUILD_VARIANT)/Release_Files $(TOP)
	$(hide) tar --checkpoint=1000 --checkpoint-action=dot -czf caas_release.tar.gz Release_Files
	$(hide) cp -r $(TOP)/caas_release.tar.gz $(TOP)/pub/$(TARGET_PRODUCT)/$(TARGET_BUILD_VARIANT)
	$(hide) rm -rf $(TOP)/caas_release.tar.gz && rm -rf $(TOP)/Release_Files
	@echo "Release files are published"
